...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f33mX = new float[numPoints];
            this.f34mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
                this.f33mX[i] = position[0];
                this.f34mY[i] = position[1];
            }
            if (((double) Math.abs(this.f33mX[0])) > 1.0E-5d || ((double) Math.abs(this.f34mY[0])) > 1.0E-5d || ((double) Math.abs(this.f33mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f34mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                StringBuilder sb = new StringBuilder();
                sb.append("The Path must start at (0,0) and end at (1,1) start: ");
                sb.append(this.f33mX[0]);
                String str = ",";
                sb.append(str);
                sb.append(this.f34mY[0]);
                sb.append(" end:");
                sb.append(this.f33mX[numPoints - 1]);
                sb.append(str);
                sb.append(this.f34mY[numPoints - 1]);
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f33mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f33mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        if (xRange == 0.0f) {
            return this.f34mY[startIndex];
        }
        float fraction = (t - fArr[startIndex]) / xRange;
        float[] fArr2 = this.f34mY;
