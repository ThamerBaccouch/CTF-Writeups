...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, OpGenerator {
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                C0223Op op = (C0223Op) this.mOps.get(opNum);
                switch (op.mCmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
            ...
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        C0223Op thatOp = (C0223Op) record.mOps.get(thoseOpIndex);
            ...
            if (i != 1) {
                if (i == 2) {
                        ...
                        if (old.mContainerId == containerId) {
                            if (old == f) {
                                ...
                                if (old == oldPrimaryNav) {
                                    this.mOps.add(opNum, new C0223Op(9, old));
                                    opNum++;
                                    oldPrimaryNav = null;
                                }
                                C0223Op removeOp = new C0223Op(3, old);
                    ...
                    if (op.mFragment == oldPrimaryNav) {
                        this.mOps.add(opNum, new C0223Op(9, op.mFragment));
                        opNum++;
                        oldPrimaryNav = null;
                    }
                } else if (i != 7) {
                    if (i == 8) {
                        this.mOps.add(opNum, new C0223Op(9, oldPrimaryNav));
                        opNum++;
    ...
    public boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (isFragmentPostponed((C0223Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isFragmentPostponed(C0223Op op) {
